...
describe("API Routes", () => {
  ...
  describe("Feed GET Route", () => {
    it("returns mock data when database is empty", async () => {
      ...
      expect(json.posts[0].author).toBe("DJ Minx");
      expect(json.hasMore).toBe(false);
    });

    it("returns database data when available", async () => {
      const dbPost = [{ id: "456", content: "DB Post", type: "GENERAL", createdAt: new Date().toISOString(), author: { name: "DB Author", role: "USER" } }];
      ...
      expect(json.hasMore).toBe(false); // Only 1 post, limit is 10
    });
  });
});
